...
public:
    MosquittoMqttClient() {
        ...
        if (!mosq_) {
            std::cerr << "[safety][MQTT] mosquitto_new failed" << std::endl;
        }
    ...
    bool connect(const std::string& host, int port,
        ...
        if (!username.empty()) {
            ...
            if (rc != MOSQ_ERR_SUCCESS) {
                std::cerr << "[safety][MQTT] username_pw_set error: " << rc << std::endl;
            }
        }
        int rc = mosquitto_connect(mosq_, host.c_str(), port, 60);
        if (rc != MOSQ_ERR_SUCCESS) {
            std::cerr << "[safety][MQTT] connect failed: " << mosquitto_strerror(rc) << std::endl;
            return false;
        }
    ...
    bool publish(const std::string& topic, const std::string& payload, int qos, bool retain) override {
        ...
        if (rc != MOSQ_ERR_SUCCESS) {
            std::cerr << "[safety][MQTT] publish error: " << mosquitto_strerror(rc) << std::endl;
            return false;
        }
        return true;
    }
    ...
    bool subscribe(const std::string& topic, int qos) override {
        ...
        if (rc != MOSQ_ERR_SUCCESS) {
            std::cerr << "[safety][MQTT] subscribe error: " << mosquitto_strerror(rc) << std::endl;
            return false;
        }
        return true;
    }
...
private:
    static void on_connect_static(struct mosquitto* m, void* obj, int rc) {
        (void)m; (void)obj;
        if (rc == 0) {
            std::cout << "[safety][MQTT] connected" << std::endl;
        } else {
            std::cerr << "[safety][MQTT] connect rc=" << rc << std::endl;
        }
    }
